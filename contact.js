...
const simpanContact = (name, email, phone) => {
  ...
  if (!name || !email || !phone) {
    ...
    validator.isMobilePhone(phone, "id-ID") === false
  ) {
...
const deleteContact = () => {
  ...
  rl.question(
    "Masukkan nama, email atau no telepon yang ingin dihapus : ",
    (input) => {
      if (
        nama.includes(input) ||
        gmail.includes(input) ||
...
const detail = (nama) => {
  rl.question("Masukkan nama yang ingin di detail : ", (nama) => {
    const contacts = loadContact();
